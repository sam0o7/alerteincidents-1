...
public class CarteActivity extends Activity {
	...
	private HashMap<Marker,Incident> marker2incident;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		marker2incident = new HashMap<Marker, Incident>();
		...
		map.setOnInfoWindowClickListener(new GoogleMap.OnInfoWindowClickListener(){
			...
			{
				Incident item = marker2incident.get(marker);
	...
	{
		List<Incident> list_Incident = data_source.getAllIncident();

		Iterator<Incident> it = list_Incident.iterator();
		int i=0;
		while (it.hasNext())
		{
	...
	//add marker on map
	private void addMarkerIncident(ArrayList<Incident> mList)
	{
		Log.v("addMarkerIncident",String.valueOf(mList.size()));
		List<Incident> list_incident = mList;//liste des incidents
		Iterator<Incident> it = list_incident.iterator();
		/*
		while (it.hasNext())
		{
